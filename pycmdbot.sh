...
if [ $IS_CTORCHIA87 -eq 0 ]; then
  python pycmdbot.py -um -c conf/ctorchia87.cfg -l log/ctorchia87.log$DATE > log/ctorchia87.out$DATE 2> log/ctorchia87.err$DATE &
fi

if [ $IS_PYCMDBOT -eq 0 ]; then
  python pycmdbot.py -a -c conf/pycmdbot.cfg -l log/pycmdbot.log$DATE > log/pycmdbot.out$DATE 2> log/pycmdbot.err$DATE &
fi
